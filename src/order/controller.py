...
def create_orders(order_data: OrderCreate, current_user: User = Depends(require_role([UserRole.SELLER, UserRole.CUSTOMER, UserRole.ADMIN])), db: Session = Depends(get_db)):
    
    return create_order(order_data, current_user, db)

@router.get("/", response_model=List[OrderResponse])
def list_all_orders(current_user: User = Depends(require_role([UserRole.SELLER, UserRole.CUSTOMER, UserRole.ADMIN])), db: Session = Depends(get_db)):
    return list_orders(current_user, db)


@router.get("/{order_id}", response_model=OrderResponse)
def get_individual_order(order_id: int, current_user: User = Depends(require_role([UserRole.SELLER, UserRole.CUSTOMER, UserRole.ADMIN])), db: Session = Depends(get_db)):
